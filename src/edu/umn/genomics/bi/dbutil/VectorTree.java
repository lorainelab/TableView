/*
 * @(#) $RCSfile: VectorTree.java,v $ $Revision: 1.2 $ $Date: 2002/07/30 19:44:46 $ $Name: TableView1_3_2 $
 *
 ...
 *
 * @author J Johnson
 * @version $Revision: 1.2 $ $Date: 2002/07/30 19:44:46 $ $Name: TableView1_3_2
 * $
 * @since 1.0
 */
public class VectorTree {
     ...
     *
     * @param branchCount The number of children for each branch of the tree.
     * @param minLevelRef The lowest level from the leaf that should be a
     * SoftReference
     * @param maxLevelRef The highest level from the leaf that should be a
     * SoftReference
     */
     ...
     */
    private Object getNode(ArrayList v, int vsize, boolean ref) {
        if (v == null) {
            v = new ArrayList(vsize);
        }
        if (v.size() != vsize) {
            v.ensureCapacity(vsize);
        }
        Object o = v;
        if (canRef && ref) {
     ...
     *
     * @param o The Object representing this node, either a Vector or a
     * SoftReference.
     * @return The Vector for this node.
     */
    private ArrayList getNodeVector(Object o) {
        if (o instanceof ArrayList) {
            return (ArrayList) o;
        } else if (canRef && o instanceof SoftReference) {
            ArrayList v = (ArrayList) ((SoftReference) o).get();
            return v;
        }
        return null;
    }
    ...
    synchronized public void setSize(int size) {
        ...
        if (newDepth > oldDepth) { // Move current root to a lower level branch
            ArrayList oldroot = root;
            root = (ArrayList) getNode(null, branchCount, false);
            ArrayList v = root;
     ...
     */
    synchronized public ArrayList get(int index) {
        if (index >= size) {
            return null;
        }
...
//if (o instanceof SoftReference) System.err.println( " cache miss " + i + " for " + index); 
    ...
    private void printNode(Object o, int level, int depth) {
        ...
        ArrayList v = getNodeVector(o);
        if (v != null) {
            for (int i = 0; i < v.size(); i++) {
                printNode(v.get(i), level + 1, depth);
            }
        }
        System.err.print(s);
        if (o instanceof ArrayList) {
