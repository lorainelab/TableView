/*
 * @(#) $RCSfile: ResultTableModel.java,v $ $Revision: 1.2 $ $Date: 2004/08/02 20:23:34 $ $Name: TableView1_3_2 $
 *
...
public class ResultTableModel extends DefaultTableModel {
     ...
     *
     * @param allowEdit whether to allow any cell editing.
     */
    ...
    public Class getColumnClass(int columnIndex) {
        if (columnClass != null && columnIndex >= 0 && columnIndex < columnClass.length) {
    ...
    public void set(ResultSet rs) throws SQLException {
        ...
        columnNames = new ArrayList(ncols);
        ...
        setDataVector((Object[][]) dataRows.toArray(), columnNames.toArray());

        for (int c = 0; c < ncols; c++) {
            columnClass[c] = colClass[c].size() == 1
                    ? (Class) (new ArrayList(colClass[c])).get(0)
                    : colClass[c].size() > 1
                    ? getCommonClass((Class[]) colClass[c].toArray(new Class[colClass[c].size()]))
    ...
    public void add(ResultSet rs) throws SQLException, Exception {
        ...
        if (ncols != getColumnCount()) {
            throw new Exception("Table columns count mismatch " + getColumnCount() + " vs " + ncols);
        }
        for (int c = 0; c < ncols; c++) {
            String colClass = rsmd.getColumnClassName(c + 1);
            if (!columnClass[c].isAssignableFrom(Class.forName(colClass))) {
                throw new Exception("Table column " + (c + 1) + " class mismatch " + columnClass[c] + " vs " + colClass);
            }
        }
        for (int r = 0; rs.next(); r++) {
