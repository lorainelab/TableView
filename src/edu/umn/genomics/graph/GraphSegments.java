/*
 * @(#) $RCSfile: GraphSegments.java,v $ $Revision: 1.7 $ $Date: 2003/07/28 15:07:10 $ $Name: TableView1_2 $
 *
 ...
 *
 * @author       J Johnson
 * @version $Revision: 1.7 $ $Date: 2003/07/28 15:07:10 $  $Name: TableView1_2 $ 
 * @since        1.0
 * @see DataModel
 */
public class GraphSegments extends AbstractGraphItem {
    ...
    if (points != null && points.length == 2 &&
      ...
      for (int i = 0; i < points[0].length; i+=2) {
        ...
        if (lineWidth == 1) {
          ...
          } else if (points[0][i] == points[0][i+1]) { // vertical line
  ...
  public BitSet getIndexBitSetIn(Rectangle r, Axis xAxis, Axis yAxis) {
    ...
    if (points != null && points[0] != null && points[1] != null) {
      ...
      int yb = r.y + r.height;
      // if a point is in the rectangle, return true
      for (int i = 0, j = 0; i < npts; i+=2, j++) {
        if (points[0][i] >= r.x && points[0][i] < xr &&
            points[1][i] >= r.y && points[1][i] < yb && 
            points[0][i+1] >= r.x && points[0][i+1] < xr &&
            points[1][i+1] >= r.y && points[1][i+1] < yb) {
          bitSet.set(j);
        }
      }
    }
  ...
  /**
   * Returns the indices of segments that intersect the given Rectangle.
