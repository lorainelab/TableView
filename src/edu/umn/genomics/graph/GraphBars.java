/*
 * @(#) $RCSfile: GraphBars.java,v $ $Revision: 1.11 $ $Date: 2004/01/28 20:32:56 $ $Name: TableView1_2 $
 *
 ...
 *
 * @author       J Johnson
 * @version $Revision: 1.11 $ $Date: 2004/01/28 20:32:56 $  $Name: TableView1_2 $ 
 * @since        1.0
 * @see DataModel
 */
public class GraphBars extends AbstractGraphItem {
   ...
   * @param barWidth the preferred bar width in axis space.
   */
  public void setBarWidth(double barWidth) {
    this.barWidth = barWidth;
    if (axes[0] != null) {
      bw = Math.abs(axes[0].getIntPosition(barWidth)-axes[0].getIntPosition(0.));
    }
    fireChangeEvent();
  }
  /**
   ...
   * @param barLabeler the labeler to generate labels over each bar.
   */
  ...
  /** 
   * The values in the index array ia are sorted by the values in the 
   * given array a.
   * @param a the array for which to generate a sorted index.
   * @param ia the sorted index generated for array a.
   * @param lo0 the lo index of the range to sort.
   * @param hi0 the high index of the range to sort.
   */
  ...
  /**
   * Return a barwidth that it is not larger than the preferred bar width 
   * and that has a space between each bar if possible.
   * @return a barwidth.
   */
  private int calcBarWidth() {
    ...
    if (points != null && points.length >= 1 &&
      ...
      if (points[0].length == 1) {
        if (axes[0] != null) {
          bw = (int)(Math.min((double)axes[0].getSize()-points[0][0],points[0][0]) * .8); 
        }
      } else {
        if (axes[0] != null) {
...
t[w++] = System.currentTimeMillis();
    if ( points == null || !r.equals(rect) || 
          !xAxis.equals(axes[0]) || !yAxis.equals(axes[1]))  {
      ...
      if (points != null && points.length >= 2 &&
        ...
        points[0].length > 0 && points[0].length == points[1].length) {
        for (int n = points[0][0], i = 1; i < points[0].length;
                 n = points[0][i],i++) {
          if (n > points[0][i]) {
            xi = getSortIndex();
            break;
...
t[w++] = System.currentTimeMillis();
      if (Double.isNaN(barWidth)) {
        ...
        bw = Math.abs(xAxis.getIntPosition(barWidth) - xAxis.getIntPosition(0.));
      }
t[w++] = System.currentTimeMillis();
    }
    if (points != null && points.length >= 2 &&
      ...
      for (int i = 0; i < points[0].length; i++) {
        ...
        if (bw < 2) {
          ...
          g.drawRect(points[0][i]-bl, points[1][i], 
                     bw-1, yb-points[1][i]);
        } else {
          g.fillRect(points[0][i]-bl, points[1][i], 
                     bw, yb-points[1][i]+1);
        }
      }
t[w++] = System.currentTimeMillis();
...
if (debug)  {
for(int i = 1; i < w; i++) 
  System.err.println(" " + i + ":\t" + (t[i] - t[i-1]) + "\t" + (t[i] - t[0]));
}
  }
  ...
  /**
   * Returns the data indices that are enclosed by the given rectangle.
   * @param r the area to test for intersection.
   * @param xAxis The X axis of the graph.
   * @param yAxis The Y axis of the graph.
   * @return A bitset of the indices of the datapoints that intersect the given rectangle.
   */
  public BitSet getIndexBitSetIn(Rectangle r, Axis xAxis, Axis yAxis) {
   ...
   * @param xAxis The X axis of the graph.
   * @param yAxis The Y axis of the graph.
   * @return indices of datapoints that intersects the given rectangle, else null.
   */
  public int[] getIndicesIn(Rectangle r, Axis xAxis, Axis yAxis) {
    BitSet bs = getIndexBitSetIn(r, xAxis, yAxis);
    int[] idx = new int[bs.cardinality()];
