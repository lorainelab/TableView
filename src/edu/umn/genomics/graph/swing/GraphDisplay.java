/*
 * @(#) $RCSfile: GraphDisplay.java,v $ $Revision: 1.9 $ $Date: 2004/01/28 20:33:00 $ $Name: TableView1_3_2 $
 *
 ...
 *
 * @author J Johnson
 * @version $Revision: 1.9 $ $Date: 2004/01/28 20:33:00 $ $Name: TableView1_3_2
 * $
 * @since 1.0
 * @see Graph
 * @see GraphItem
...
public class GraphDisplay extends JComponent implements Graph {
    ...
    ArrayList graphItems = new ArrayList();
  ...
  private MouseAdapter ma = new MouseAdapter() {
     ...
     *
     * @param gridColor the color for grid lines on the graph.
     */
     ...
     *
     * @param showRubberband whether to show the selecting rectangle on the
     * display.
     */
    ...
    public GraphItem[] getGraphItems() {
        ...
        for (int i = 0; i < graphItems.size(); i++) {
            gia[i] = (GraphItem) graphItems.get(i);
        }
        return gia;
    }
    ...
    public void paintComponent(Graphics g) {
        ...
        for (int i = 0; i < graphItems.size(); i++) {
            ((GraphItem) graphItems.get(i)).draw(this, g, dataArea, xAxis, yAxis);
        }
        if (sPt != null && ePt != null) {
            g.setColor(getForeground());
            g.drawRect(Math.min(sPt.x, ePt.x), Math.min(sPt.y, ePt.y),
