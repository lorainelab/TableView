/*
 * @(#) $RCSfile: AxisDisplay.java,v $ $Revision: 1.11 $ $Date: 2004/01/28 20:32:59 $ $Name: TableView1_2 $
 *
 ...
 *
 * @author       J Johnson
 * @version $Revision: 1.11 $ $Date: 2004/01/28 20:32:59 $  $Name: TableView1_2 $ 
 * @since        1.0
 */
public class AxisDisplay extends JComponent implements AxisComponent {
    ...
    public void mouseDragged(MouseEvent e) {
      ...
      if (v2 != v1) {
        ...
        if ( (evtMask == InputEvent.BUTTON2_MASK) ||
             ...
             (evtMask == InputEvent.SHIFT_MASK) ) {  // Mac OneButtonMouse
          double diff = (getAxis().getValue(v1) - getAxis().getValue(v2));
  ...
  private MouseAdapter ma = new MouseAdapter() {
    ...
    public void mousePressed(MouseEvent e) {
      ...
      if ( (evtMask == InputEvent.BUTTON1_MASK) ||
        ...
        zoomOp = ZOOMPAN;
             ...
             (evtMask == (InputEvent.CTRL_MASK|InputEvent.BUTTON2_MASK)) ||  // Mac 
             (evtMask == InputEvent.CTRL_MASK || 
              evtMask == InputEvent.META_MASK) ) { // Mac OneButtonMouse
        zoomOp = ZOOMOUT;
      }
      repaint();
    }
    ...
    public void mouseWheelMoved(MouseWheelEvent e) {
        ...
        Axis axis = getAxis();
        double min = axis.getValue(rotation);
        double max = axis.getValue(axis.getSize()+ rotation - 1);
        axis.setMin(min);
        axis.setMax(max);
        System.err.println("rpt:" + rotation + " min:"+ min + " max:" + max);
        repaint();
    }
  };
 ...
 // zoom out one pixel on each edge
  ...
  /**
   * Create an AxisDisplay with the given axis and default position 
   ...
   * @param zoom if true allow zooming else axis is fixed.
   */
  public void setZoomable(boolean zoom) {
    if (zoom && !iszoomable) {
      ...
      getInputMap().put(KeyStroke.getKeyStroke(KeyEvent.VK_DOWN,0,false), ZOOM_OUT);
      getActionMap().put(ZOOM_OUT, zoomoutAction);
      getInputMap().put(KeyStroke.getKeyStroke(KeyEvent.VK_UP,0,false), ZOOM_IN);
      getActionMap().put(ZOOM_IN, zoominAction);
      getInputMap().put(KeyStroke.getKeyStroke(KeyEvent.VK_LEFT,0,false), PAN_UP_OR_LEFT);
      getActionMap().put(PAN_UP_OR_LEFT, panUpLeftAction);
      getInputMap().put(KeyStroke.getKeyStroke(KeyEvent.VK_RIGHT,0,false), PAN_DOWN_OR_RIGHT);
  ...
  public double getValueAt(double x, double y) {
    double v;
    double p = position == LEFT ||  position == RIGHT ? getSize().height - getInsets().bottom - y : x;
  ...
  public Dimension getPreferredSize() {
    ...
    if (axis != null) {
      ...
      case BOTTOM:
        ...
        d.height = tagheight + tickLen + gap + insets.top + insets.bottom;
        break;
      case LEFT:
      case RIGHT:
        d.width = tagwidth + tickLen + gap*2 + insets.left + insets.right;
  ...
  public void paintComponent(Graphics g) {
    ...
    Insets insets = getInsets();
    dataArea = new Rectangle(insets.left, insets.top,
