/*
 * @(#) $RCSfile: SimpleGraph.java,v $ $Revision: 1.7 $ $Date: 2003/08/20 13:58:33 $ $Name: TableView1_2 $
 *
...
/**
 * A graph container containing a Graph display and optional axis displays 
 * along any border.
 * @author       J Johnson
 * @version $Revision: 1.7 $ $Date: 2003/08/20 13:58:33 $  $Name: TableView1_2 $ 
 * @since        1.0
 * @see  GraphItem
 * @see  Axis
 */
public class SimpleGraph extends JPanel implements Graph {
 ...
 // toggle between log and linear axis
  ...
  private void addLogToggle(AxisDisplay axisD) {
    axisD.getInputMap().put(KeyStroke.getKeyStroke(KeyEvent.VK_ESCAPE,0,false), logAxisToggle);
    axisD.getActionMap().put(logAxisToggle, logToggleAction);
    axisD.getInputMap().put(KeyStroke.getKeyStroke(KeyEvent.VK_I,0,false), invertAxisToggle);
  ...
  public void alterView(String command) {
    ...
    public void alterView(Point p, int amount) {
        double aspect = (double) graph.getWidth() / (double) graph.getHeight();
        for (int i = 0; i < 4; i++) {
            AxisDisplay axisD = i == 0 ? laxis
                    : i == 1 ? raxis
                    : i == 2 ? taxis
            ...
            if (axisD != null && axisD.getZoomable()) {
                ...
                double maxVal;
                if (p != null) {                     
                    double f = i < 2 ? 1 - p.getY() / axis.getSize() : p.getX() / axis.getSize();
                    minVal = min -  range * adj * f;
                    maxVal = max +     range * adj * (1 - f);   
                } else {
...
// move one pixel up or left
 ...
 // zoom out one pixel on each edge
  ...
  private void addGraphKeys(GraphDisplay graph) {
    ...
    graph.addMouseWheelListener(gma);
    graph.getInputMap().put(KeyStroke.getKeyStroke(KeyEvent.VK_DOWN,0,false), ZOOM_OUT);
    graph.getActionMap().put(ZOOM_OUT, zoomoutAction);
    graph.getInputMap().put(KeyStroke.getKeyStroke(KeyEvent.VK_UP,0,false), ZOOM_IN);
    graph.getActionMap().put(ZOOM_IN, zoominAction);
    graph.getInputMap().put(KeyStroke.getKeyStroke(KeyEvent.VK_LEFT,0,false), PAN_UP_OR_LEFT);
    graph.getActionMap().put(PAN_UP_OR_LEFT, panUpLeftAction);
    graph.getInputMap().put(KeyStroke.getKeyStroke(KeyEvent.VK_RIGHT,0,false), PAN_DOWN_OR_RIGHT);
    graph.getActionMap().put(PAN_DOWN_OR_RIGHT, panDownRightAction);
  }

  /**
   * Create a graph with a graph display and axis displays along the 
   ...
   * @param borderPosition the border along the graph display
   * @param show whether the axis should be displayed at the given position.
