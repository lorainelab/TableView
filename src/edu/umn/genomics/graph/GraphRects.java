/*
 * @(#) $RCSfile: GraphRects.java,v $ $Revision: 1.6 $ $Date: 2004/01/28 20:32:58 $ $Name: TableView1_2 $
 *
...
/**
 * GraphRects draws rectangles on a graph from a data array such that 
 * rectangle n has one corner at X,Y position data[n*2], data[n*2+1]
 * and the other corner at X,Y position data[n*2+2], data[n*2+3];
 *
 * @author       J Johnson
 * @version $Revision: 1.6 $ $Date: 2004/01/28 20:32:58 $  $Name: TableView1_2 $ 
 * @since        1.0
 * @see DataModel
 */
public class GraphRects extends AbstractGraphItem {
   ...
   * @param fill fill if true else draw outline.
   */
   ...
   * @return true if rectangle are drawn filled else false if drawn as outline.
   */
  ...
  public void draw(Component c, Graphics g, Rectangle r, Axis xAxis, Axis yAxis) {
    ...
    if (points != null && points.length >= 2 &&
      ...
      for (int i = 0; i + 1 < points[0].length; i+=2) {
        ...
        int x = points[0][i] < points[0][i+1] ? points[0][i] : points[0][i+1];
        int y = points[1][i] < points[1][i+1] ? points[1][i] : points[1][i+1];
  ...
  /**
   * Returns whether this graph item intersects the given point.
   * @param r the rectagle to test for intersection.
   * @param xAxis The X axis of the graph.
   * @param yAxis The Y axis of the graph.
   * @return indices of datapoints that intersects the given point, else null.
   */
