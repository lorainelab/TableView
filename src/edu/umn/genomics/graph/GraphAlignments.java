/*
 * @(#) $RCSfile: GraphAlignments.java,v $ $Revision: 1.5 $ $Date: 2004/01/28 20:32:56 $ $Name: TableView1_3_2 $
 *
 ...
 *
 * @author J Johnson
 * @version $Revision: 1.5 $ $Date: 2004/01/28 20:32:56 $ $Name: TableView1_3_2
 * $
 * @since 1.0
 * @see DataModel
 */
public class GraphAlignments extends AbstractGraphItem {
    ...
    /*
     * private void positionItems() { if (points != null && points.length > 1 &&
     * points[0] != null) { ArrayList v = new ArrayList(); BitSet clrBit = new
     * BitSet(); BitSet curBit = new BitSet();
     *
     * int cnt[] = new int[r.width]; for (int i = 0; i < points[0].length; i+=2)
     * { int xs; int xe; if ( points[0][i] <= points[0][i+1]) { xs =
     * points[0][i]; xe = points[0][i+1]; } else { xs = points[0][i+1]; xe =
     * points[0][i]; } int w = xe - xs; w = w < 2 ? 2 : w; int cm = 0; if (xs <
     * r.width && xe >= 0) { curBit.and(clrBit); for (int j = xs <= 0 ? 0 :
     * xs-1; j <= xe+1 && j < r.width; j++) { curBit.set(j); } boolean placed =
     * false; for (int j = 0; j < v.size(); j++) { BitSet row =
     * (BitSet)v.elementAt(j); if
     * (((BitSet)curBit.clone()).and(row).equals(clrBit)) { cm = j;
     * row.or(curBit); placed = true; break; } } if (!placed) {
     * v.add((BitSet)curBit.clone()); cm = v.size(); }
     *
     * int y = r.height - cm * (lineWidth + 1); if (points[1] != null && i <
     * points[1].length) { points[1][i] = y; points[1][i+1] = y; } } } } }
     */
    /**
