/*
 * @(#) $RCSfile: LogAxis.java,v $ $Revision: 1.5 $ $Date: 2004/01/28 20:32:59 $ $Name: TableView1_3_2 $
 *
...
import java.util.*;
/*
 * Maps min -> 1 and  max -> base  which is then scaled and offset
 * size = 100 pixels
 * min = 0 
 * max = 30
 * 
...
/**
 * The LogAxis class provides a transformation between a value plotted 
 * along an axis to a pixel position.
 * @author       J Johnson
 * @version $Revision: 1.5 $ $Date: 2004/01/28 20:32:59 $  $Name: TableView1_3_2 $ 
 * @since        AV1.0
 */
public class LogAxis extends AbstractAxis implements Cloneable {
  ...
  /** 
   * Create a LogAxis with default values: 
  ...
  /** 
   * Create a LogAxis with default values of min = -10, max = 10 
  ...
  /** 
   * Create a LogAxis with the same min, max, and size values. 
   * as the given axis.
   ...
   * @param base the logarithmic base for calculating positions.
   * @throws IllegalArgumentException If base <= 0.
   */
  public void setBase(double base) throws IllegalArgumentException {
    if (base <= 0.) {
      throw new IllegalArgumentException("base value " +  base + " is not greater than 0.");
    }
    this.base = base;
  }
  ...
  protected double valueAt(int position) {
    return min + (max-min)*.1*
      (Math.exp((double)position/(double)size*Math.log(base))-1)*(base/(base-1));
  }
