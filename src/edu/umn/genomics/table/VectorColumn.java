/*
 * @(#) $RCSfile: VectorColumn.java,v $ $Revision: 1.1 $ $Date: 2004/08/02 20:23:47 $ $Name: TableView1_3_2 $
 *
 ...
 * 
 * @author       J Johnson
 * @version $Revision: 1.1 $ $Date: 2004/08/02 20:23:47 $  $Name: TableView1_3_2 $ 
 * @since        1.0
...
public class VectorColumn implements VirtualColumn {
   ...
   * @param columnClass The java class that all values for this column must extend, 
   ...
   * @param columnClass The java class that all values for this column must extend, 
   *  if null there is no restriction.
   * @param values The data values for the column.
   */
  ...
  public Class getColumnClass() {
    return columnClass != null 
             ? columnClass 
             : commonColumnClasses != null && commonColumnClasses.size() > 0 
               ? (Class)commonColumnClasses.get(0) 
   ...
   * @param b Whether cells in this column should be editable.
   */
      ...
      if (columnClass != null) {
        if (aValue != null && !columnClass.isAssignableFrom(aValue.getClass())) {
          throw new ClassCastException(aValue + " (" 
                      + aValue.getClass() + ") can not be assigned to " + columnClass);
        }
  ...
  public String getType() {
    ...
    return theClass != null 
             ? theClass.getName().substring(theClass.getPackage().getName().length()+1)
             : "Unknown";
  }
