/*
 * @(#) $RCSfile: IndexMapSelection.java,v $ $Revision: 1.6 $ $Date: 2003/04/15 15:47:55 $ $Name: TableView1_3_2 $
 *
...
/**
 * IndexMapSelection bridges two ListSelectionModels,
 * so that selections in either will be reflected in the other.  
 * This adds a listener to each ListSelectionModel and responds to 
 * selection events by altering the selection in the other ListSelectionModel.
 * An IndexMap can provide a mapping between the indices of the 
 * two ListSelectionModels.
 *
 * @author       J Johnson
 * @version $Revision: 1.6 $ $Date: 2003/04/15 15:47:55 $  $Name: TableView1_3_2 $ 
 * @since        1.0
 */
public class IndexMapSelection implements CleanUp {
   ...
   * reflected in the other.  If a map is given, it provides the mapping 
   * of indices between the two ListSelectionModels.
   * @param model1 One of the ListSelectionModels to link together. 
   * @param model2 The other ListSelectionModels to link together.
   * @param map If not null, this provides the mapping of indices between 
   *   the two ListSelectionModels.
   */
  public IndexMapSelection(ListSelectionModel model1, ListSelectionModel model2, 
                           IndexMap map) {
   ...
   */
  public IndexMapSelection(ListSelectionModel model1, ListSelectionModel model2, 
                           IndexMap map, boolean allSelected) {
    this.src = model1;
    this.dst = model2;
    this.map = map;
    this.allSelected = allSelected;
    if (src.getMinSelectionIndex() >= 0) {
      selListener.valueChanged(new ListSelectionEvent(src,
                                          src.getMinSelectionIndex(),
   ...
   * 
   * @param allSelected This only applies to OneToManyIndexMaps, if true all 
   * destination indices must be selected in order for the source index to be 
   * selected, if false then the source index is selected if any of its associated 
   * destination indices are selected.
   */
