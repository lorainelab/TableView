/*
 * @(#) $RCSfile: ColumnGraph.java,v $ $Revision: 1.6 $ $Date: 2002/07/30 19:45:01 $ $Name: TableView1_3_2 $
 *
 ...
 */
public class ColumnGraph extends AbstractTableModelView
  ...
  private MouseMotionAdapter mma = new MouseMotionAdapter() {
    public void mouseDragged(MouseEvent e) {
      ...
      value.setText(((int)v[0]) + "," + 
        (ctx.getColumnMap(tm,gds.indexOf(gd)).getMappedValue(v[1],0)));
    }
...
/*
      ...
      gpanel.remove(tc);
      gpanel.add(tc,0); // need extents
      tpanel.remove(c);
      tpanel.add(c,0); // need extents
*/
      repaint();
    }
    public void mousePressed(MouseEvent e) {
  ...
  private void initListeners() {
    ...
    JLabel rowLbl = new JLabel("Row #",JLabel.RIGHT);
    jsp.setCorner(JScrollPane.UPPER_LEFT_CORNER,rowLbl);
  ...
  /**
   * Constructs a ColumnGraph display.  Nothing will be displayed 
  ...
  /**
   * Constructs a ColumnGraph diaplay which is initialized with 
   ...
   * performed by the ColumnGraph when it receives the event.
  ...
  */
          ...
          gd.setBorder(BorderFactory.createEmptyBorder(
            insets.top, ginsets.left, insets.bottom, ginsets.right));
          ip.addMouseListener(pma);
          ip.addMouseMotionListener(pmma);
          ip.add(tab,BorderLayout.WEST);
          AxisDisplay vaxis = new AxisDisplay(yAxis,AxisComponent.LEFT);
...
//System.err.println("ti " + lw + " " + ppr + " " + lc + " " + tn + " " + nti + " " + exp);
  ...
  public void paintComponent(Graphics g) {
    ...
    if (!lsm.isSelectionEmpty()) {
      ...
      int xoff = axisDisp.getLocationOnScreen().x - getLocationOnScreen().x 
                 + axisDisp.getInsets().left;
