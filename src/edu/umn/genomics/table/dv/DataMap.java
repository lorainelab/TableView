/*
 * @(#) $RCSfile: DataMap.java,v $ $Revision: 1.14 $ $Date: 2003/07/28 15:07:19 $ $Name: TableView1_3_2 $
 *
...
/*
 * Some mapping to consider for the table: points - one point per table row
 * polyline - connecting the points area - shaded area under (over) a polyline
 * lines - one point per table row bars stacked bars glyphs text surface - 3D
 * volume - 3D
 */
/**
 * This class maps columns from a TableModel to displayable attributes. It
 * implements TableModelListener so that it can change the displayed shapes when
 * the data is changed. This is a base class that should be extended for
 * particular displays such as a 2D or 3D mapping.
 *
 * @author James E Johnson
 * @version $Revision: 1.14 $ $Date: 2003/07/28 15:07:19 $ $Name: TableView1_3_2
 * $
 * @since dv1.0
 */
public class DataMap extends Observable
    ...
    /**
     * Attempt a default mapping of TableColumns to displayable attributes. The
     * first three numerically valued columns are assigned to x,y,z axes. A
     * fourth numerically valued column is assigned to a color value. The first
     * String valued column is used as a label for this data point.
     */
    public void autoMap() {
    ...
    public float[] getPointColor(int rowIndex, int rowCount, float colors[],
        ...
        for (int r = rowIndex, p = colorOffset; r < rmax; r++, p += colorIncrement) {
            ...
            System.arraycopy(ca, 0, colors, p, colorIncrement);
        }
        return colors;
    }
    ...
    public void valueChanged(ListSelectionEvent e) {
        ...
        /*
         * int min = e.getFirstIndex(); int max = e.getLastIndex(); for (int i =
         * min; i < max; i++) { selectDataPoint(i,lsm.isSelectedIndex(i)); }
         */
        setChanged();
        notifyObservers(lsm);
    }
    ...
    /**
     * Called when selection status of a datapoint changes. SubClasses need to
    ...
    /**
     * Return an array of indices associated with column given its name. If a
     * name isn't found, the index is set to -1.
     *
    ...
    /*
     * public void setPositionSource(int column[]) { } public void
     * setColorSource(int column) { } public void setTransparencySource(int
     * column) { } public void setScaleSource(int column) { } public void
     * setScaleSource(int column[]) { } public void setColorMap() { }
     */
}
