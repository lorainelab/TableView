/*
 * @(#) $RCSfile: Matrix4D.java,v $ $Revision: 1.4 $ $Date: 2002/07/30 19:45:20 $ $Name: TableView1_3_2 $
 *
...
/**
 * A 4D matrix object that can transform sets of 4D points and perform a variety
...
class Matrix4D implements Serializable {
    ...
    public void transform(float v[], float tv[], int nvert) {
        ...
        for (int i = nvert * 3; (i -= 3) >= 0;) {
            ...
            float y = v[i + 1];
            float z = v[i + 2];
            tv[i] = (float) (x * lxx + y * lxy + z * lxz + lxw);
            tv[i + 1] = (float) (x * lyx + y * lyy + z * lyz + lyw);
            tv[i + 2] = (float) (x * lzx + y * lzy + z * lzz + lzw);
        }
    }
    ...
    public void transform(float v[], int inc, float tv[], int outc, int first, int nvert) {
        ...
        for (int i = (first + nvert) * inc, j = (first + nvert) * outc;
            ...
            float z = v[i + 2];
            if (inc == 3) {
                tv[j] = (float) (x * lxx + y * lxy + z * lxz + lxw);
                tv[j + 1] = (float) (x * lyx + y * lyy + z * lyz + lyw);
                tv[j + 2] = (float) (x * lzx + y * lzy + z * lzz + lzw);
                if (outc == 4) {
                    tv[j + 3] = 1f;
                }
            } else if (inc == 4) {
                float w = v[i + 3];
                tv[j] = (float) (x * lxx + y * lxy + z * lxz + w * lxw);
                tv[j + 1] = (float) (x * lyx + y * lyy + z * lyz + w * lyw);
                tv[j + 2] = (float) (x * lzx + y * lzy + z * lzz + w * lzw);
                if (outc == 4) {
                    tv[j + 3] = (float) (x * lwx + y * lwy + z * lwz + w * lww);
                }
            }
        }
    }
