/*
 * @(#) $RCSfile: DataView3D.java,v $ $Revision: 1.10 $ $Date: 2002/07/30 19:45:17 $ $Name: TableView1_3_2 $
 *
...
/**
 * @author J Johnson
 * @version $Revision: 1.10 $ $Date: 2002/07/30 19:45:17 $ $Name: TableView1_3_2
 * $
 * @since 1.0
 */
public class DataView3D extends DataView implements Serializable {
        ...
        public void mouseDragged(MouseEvent e) {
            ...
            if (viewAction == NAVIGATE) {
                ...
                if ((evtMask == InputEvent.BUTTON1_MASK)
                        || evtMask == 0) {  // Mac
                    float dx = (float) ((p.x - lastPoint.x) * xRot);
                    float dy = (float) ((p.y - lastPoint.y) * -yRot);
                    ...
                    if (p.y != lastPoint.y) {
                        int h = getHeight();
                        mvm.translate(0f, 0f, (float) ((lastPoint.y - p.y) * -zTrans));
                        transformPoints(lastdm);
                    }
                    ...
                    mvm.translate((float) ((lastPoint.x - p.x) * -xTrans),
                            (float) ((lastPoint.y - p.y) * yTrans),
                            0f);
                    transformPoints(lastdm);
                }
...
//System.err.println("px: " + (t1 - t0)); 
    ...
    public DataMap addDataMap(String name, TableModel tableModel) {
        DataMap3D dm = new DataMap3D(name, tableModel);
        // add entry to hashtable
        DataMap3D prev = (DataMap3D) dataMapName.put(name, dm);
    ...
    /*
     * transform points for 2d screen // modelview transform // projection
     * transform // perspective division // viewport tranform // sort back to
     * front
     */
    private void transformPoints(DataMap3D dm) {
        if (dm == null) {
            return;
        }
...
//System.err.println("tx: " + (t5 - t0) + " " + (t1 - t0) + " " 
//+ (t2 - t1) + " " + (t3 - t2) + " " + (t4 - t3) + " " + (t5 - t4) + " " );
    ...
    private void quickSort(int a[], int left, int right) {
        ...
        if (right > left) {
            ...
            while (leftIndex <= rightIndex) {
                 ...
                 * partitionIndex starting from the leftIndex.
                 */
                while ((leftIndex < right)
                        && (compare(a[leftIndex], a[partitionIndex]) < 0)) {
                    ++leftIndex;
                }
                /*
                 * find an element that is smaller than or equal to the
                 * partitionIndex starting from the rightIndex.
                 */
                while ((rightIndex > left)
                        && (compare(a[rightIndex], a[partitionIndex]) > 0)) {
                    --rightIndex;
                }
    ...
    public void update(Observable o, Object arg) {
        if (arg instanceof TableModel) {
            transformPoints(lastdm);
            repaint();
        } else if (arg instanceof ListSelectionModel) {
            transformPoints(lastdm);
            repaint();
        }
    }
