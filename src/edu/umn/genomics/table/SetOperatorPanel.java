/*
 * @(#) $RCSfile: SetOperatorPanel.java,v $ $Revision: 1.6 $ $Date: 2003/09/17 16:25:21 $ $Name: TableView1_3_2 $
 *
...
/**
 * The SetOperatorPanel displays a set of RadioButtons to choose the 
 * Set operator.
 * @author       J Johnson
 * @version $Revision: 1.6 $ $Date: 2003/09/17 16:25:21 $  $Name: TableView1_3_2 $ 
 * @since        1.0
 * @see SetOperator
 */
public class SetOperatorPanel extends JPanel implements Serializable, ChangeListener {
  ...
  /**
   * Construct a panel of RadioButtons that will use setOperator as the 
   * model for maintaining the Set operator choice.
   */
  public SetOperatorPanel() { 
    ...
    sBrushOver = new JRadioButton(
                new SetIcon(Color.gray,Color.white,
                            SetOperator.BRUSHOVER));
    sBrushOver.setMargin(new Insets(1,1,1,1));
    sBrushOver.setSelectedIcon(
                new SetIcon(SetOperator.BRUSHOVER));
    ...
    add(sBrushOver);
    sReplace = new JRadioButton(
                new SetIcon(Color.gray,Color.white,
                            SetOperator.REPLACE));
    sReplace.setMargin(new Insets(1,1,1,1));
    sReplace.setSelectedIcon(
                new SetIcon(SetOperator.REPLACE));
    sReplace.setSelected(curOp == SetOperator.REPLACE);
    sReplace.setToolTipText("Replace previous selection with the new selection");
    ...
    group.add(sReplace);
    add(sReplace);
    sUnion = new JRadioButton(
                new SetIcon(Color.gray,Color.white,SetOperator.UNION));
    sUnion.setMargin(new Insets(1,1,1,1));
    sUnion.setSelectedIcon(
                new SetIcon(SetOperator.UNION));
    sUnion.setSelected(curOp == SetOperator.UNION);
    //sUnion.setPressedIcon(new SetIcon(SetOperator.UNION));
    sUnion.setToolTipText("Union previous selection with the new selection");
    ...
    group.add(sUnion);
    add(sUnion);
    sIntersect = new JRadioButton(
                new SetIcon(Color.gray,Color.white,
                            SetOperator.INTERSECTION));
    sIntersect.setMargin(new Insets(1,1,1,1));
    sIntersect.setSelectedIcon(
                new SetIcon(SetOperator.INTERSECTION));
    sIntersect.setSelected(curOp == SetOperator.INTERSECTION);
    sIntersect.setToolTipText("Intersect previous selection with the new selection");
    ...
    add(sIntersect);
    sDiff = new JRadioButton(
                new SetIcon(Color.gray,Color.white,
                            SetOperator.DIFFERENCE));
    sDiff.setMargin(new Insets(1,1,1,1));
    sDiff.setSelectedIcon(
                new SetIcon(SetOperator.DIFFERENCE));
    sDiff.setSelected(curOp == SetOperator.DIFFERENCE);
    sDiff.setToolTipText("Delete the new selection from the previous selection");
    ...
    group.add(sDiff);
    add(sDiff);
    sXOR = new JRadioButton(
                new SetIcon(Color.gray,Color.white,
                            SetOperator.XOR));
    sXOR.setMargin(new Insets(1,1,1,1));
    sXOR.setSelectedIcon(
                new SetIcon(SetOperator.XOR));
    sXOR.setSelected(curOp == SetOperator.XOR);
    sXOR.setToolTipText("Exclusive OR of previous selection with new selection");
   ...
   * Construct a panel of RadioButtons that will use setOperator as the 
   * model for maintaining the Set operator choice.
   * @param setOperator the object that will maintain the set operator choice.
   */
  ...
  public void setSetOperators(int setOperand) {
    for (Iterator iter = setOps.keySet().iterator(); iter.hasNext(); ) {
      Object o = (SetOperator)iter.next();
   ...
   * @param e the notification that the SetOperator has changed.
   */
