/*
 * @(#) $RCSfile: ColumnMapBinModel.java,v $ $Revision: 1.4 $ $Date: 2004/09/21 17:32:18 $ $Name: TableView1_3_2 $
 *
...
/**
 * ColumnMapBinModel partitions the values in a ColumnMap 
...
public class ColumnMapBinModel implements MutableBinModel {
  ...
  BinLabeler defaultBinLabeler = new BinLabeler() {
      public String getLabel(int binIndex) {
        ...
        case 5:
          break;
        default:
            double dmin = binMin != null && binIndex < binMin.length 
                          ? binMin[binIndex] : getBinMin(binIndex); 
            double dmax = binMax != null && binIndex < binMax.length 
                          ? binMax[binIndex] : getBinMax(binIndex); 
            Object min = cmap.getMappedValue(binMin != null && binIndex < binMin.length 
                          ? binMin[binIndex] : getBinMin(binIndex),1); 
            Object max = cmap.getMappedValue(binMax != null && binIndex < binMax.length 
                          ? binMax[binIndex] : getBinMax(binIndex),-1); 
            // Object max = cmap.getMappedValue(getBinMax(binIndex),-1); 
            Object minNext = binIndex + 1 < getBinCount() 
                              ? cmap.getMappedValue(binMin != null && binIndex+1 < binMin.length 
                                 ? binMin[binIndex+1] : getBinMin(binIndex+1),1)
                              : max; 
            // System.err.println("lbl " + binIndex + "\t" + min + "\t" + max + "\t" + dmin + "\t" + dmax + "\t" + binMin);
            if ((min != null && min.equals(max)) 
                 // || (max != null && max.equals(minNext))
              ) {
              return min.toString();
              // return binIndex >= getBinCount() - 1 ? max.toString() : min.toString();
            }
            return min + "-" + max;
        }
        return "" + (binIndex + 1);
   ...
   * @param cmap the ColumnMap to partition into bins.
   */
  ...
  /** 
   * Return the minimum map value that can be placed in 
   * the bin with the given index. All values palce in this bin are 
  ...
  public double getBinMin(int binIndex) {
    ...
    case 0:
      return binIndex;
    case 5:
      return binMin != null && binIndex < binMin.length ? binMin[binIndex] : binIndex + 0.;
    }
    return getDividers()[binIndex];
  }
  /** 
   * Return the maximum map value that can be placed in 
   * the bin with the given index. All values palce in this bin are 
  ...
  public double getBinMax(int binIndex) {
    ...
    case 0:
      return binIndex + 0.;
    case 5:
      return binMax != null && binIndex < binMax.length ? binMax[binIndex] : binIndex + 0.;
    }
    return getDividers()[binIndex+1];
  }
  /**
   * Return the array of values that divide the bins.
   * This will be null for a PartitionIndexMap.
   ...
   * to the given value.
   * @param val The map value in the ColumnMap.
  ...
  public int getBinSize(int binIndex) {
    ...
    return binCnts != null && binIndex >= 0 && binIndex < binCnts.length ? binCnts[binIndex] : 0; 
  }
