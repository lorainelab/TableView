/*
 * @(#) $RCSfile: Histogram.java,v $ $Revision: 1.9 $ $Date: 2004/05/13 20:11:23 $ $Name: TableView1_3_2 $
 *
 ...
 *
 * @author J Johnson
 * @version $Revision: 1.9 $ $Date: 2004/05/13 20:11:23 $ $Name: TableView1_3_2
 * $
 * @since 1.0
 */
public class Histogram extends JComponent implements Serializable {
     ...
     *
     * @param counts the number of elements in each bucket of the histogram
     * @param withSelectCounts the counts include selected number in each bucket
     */
     ...
     *
     * @param axis Vertical axis for scaling bars
     */
    ...
    public void paintComponent(Graphics g) {
        ...
        if (cnts != null) {
            ...
            double scale = range > 0 ? (double) (h - 2) / (double) range : 1.;
            // scale factor along the x axis
            double xs = cnts.length > 1 ? (double) w / (cnts.length / offset) : 0;
            // draw axis
            g.setColor(Color.black);
            g.drawLine(xl, yb, xr, yb);
            // draw histogram bars
            for (int i = 0; i < cnts.length; i += offset) {
                ...
                if (cnts[i] > 0) {
                    ...
                    if (doSelectCounts) {
                        if (cnts[i] > 0) {
                            y1 = y0 - (int) (min + cnts[i] * scale
                                    * (1. - (double) cnts[i + 1] / (double) cnts[i]));
                        }
                    }
