/*
 * @(#) $RCSfile: JDBCTableModel.java,v $ $Revision: 1.14 $ $Date: 2004/05/13 14:42:44 $ $Name: TableView1_3_2 $
 *
 ...
 *
 * @author J Johnson
 * @version $Revision: 1.14 $ $Date: 2004/05/13 14:42:44 $ $Name: TableView1_3_2
 * $
 * @since 1.0
 */
public class JDBCTableModel extends AbstractTableModel implements TableColumnMap {
    ...
    ArrayList activeStatements = new ArrayList();
    // Table info
    String query = null;
    // ResultSet rs = null;
    int rowCount = 0;
    ...
    HashMap statThreads = new HashMap(); // ColumnMap -> Thread
    // Statement List
    private ArrayList stmtv = new ArrayList();
    // ResultSet List
    private ArrayList rsltv = new ArrayList();
    // Count Thread
     ...
     * @param pageSize The number of rows to retrieve. (The SQL99 LIMIT
     * parameter)
     * @param page The page of rows to retrieve (starting from page 1). (The
        ...
        if (dbparams != null) {
            ...
            dataThread = new Thread() {
                ...
                public void run() {
                    try {
                        if (debug > 0) {
                            System.err.println("dataThread " + "\t" + getQuery());
                        }
    ...
    private void addStatement(Statement stmt) {
        ...
        try {
            if (debug > 3) {
                System.err.println("stmtv add: " + stmt.getConnection() + " " + stmtv);
            }
        } catch (Exception ex) {
            if (debug > 3) {
    ...
    private void addResultSet(ResultSet rslt) {
        ...
        try {
            if (debug > 3) {
                System.err.println("rsltv add: " + rslt.getStatement().getConnection() + " " + rsltv);
            }
        } catch (Exception ex) {
            if (debug > 3) {
    ...
    private Thread collectStatsThread(final DBColumnMap cMap) {
        Thread statsThread = new Thread() {
            ...
            public void run() {
                try {
                    if (debug > 3) {
                        System.err.println("statsThread " + "\t" + cmap.getName());
                    }
                    collectStats(cmap);
                } catch (Exception ex) {
