/*
 * @(#) $RCSfile: Histogram3dDisplay.java,v $ $Revision: 1.1 $ $Date: 2004/08/02 20:23:40 $ $Name: TableView1_3 $
 *
...
public class Histogram3dDisplay extends JComponent {
  ...
  public int getDecimalPlaces(double num) {
    return num == 0. ? 1 : 1 + (int)Math.floor(Math.log(Math.abs(num))/Math.log(10.));
  }
  ...
  /*
      // Determine the increments for counts
  ...
  static int[][] facets = {
    ...
    {TOP_FACET,FRONT_FACET,LEFT_FACET,FRONT_SEL_FACET,LEFT_SEL_FACET},
    {TOP_FACET,FRONT_FACET,RIGHT_FACET,FRONT_SEL_FACET,RIGHT_SEL_FACET},
    {TOP_FACET,RIGHT_FACET,BACK_FACET,RIGHT_SEL_FACET,BACK_SEL_FACET},
  ...
  public int[] visibleFacets(Point4d[] coords, int binCnt, int selCnt, boolean selectOnly) {
    /*
    // selectBarsOnly
         selCnt == 0       // 4 points
         else              // 8 points
    ...
    */
    if (coords == null) {
      ...
      case 0:
        return coords.length == 12 ? facets[16] 
                                   : selectBarsOnly || selCnt == binCnt ? facets[8] 
                                                                        : facets[0];
      case 1:
        return coords.length == 12 ? facets[17] 
                                   : selectBarsOnly || selCnt == binCnt ? facets[9] 
                                                                        : facets[1];
      case 2:
        return coords.length == 12 ? facets[18] 
                                   : selectBarsOnly || selCnt == binCnt ? facets[10] 
                                                                        : facets[2];
      case 3:
        return coords.length == 12 ? facets[19] 
                                   : selectBarsOnly || selCnt == binCnt ? facets[11] 
                                                                        : facets[3];
      case 4:
        return coords.length == 12 ? facets[16] 
                                   : selectBarsOnly || selCnt == binCnt ? facets[8] 
    ...
    for (int i = 0, pos = 0; i < binCount; i++, pos += POLYS_PER_BAR) {
      ...
      if (selectBarsOnly) {
        if (selCnt == 0) {  // 4 points
          ...
          int[] facet = visibleFacets(pnts4, binCnt, selCnt, selectBarsOnly);
          ...
          int[] facet = visibleFacets(pnts8, binCnt, selCnt, selectBarsOnly);
          for (int j = 0; j < facet.length; j++) {
            poly = getPolygon(pnts, facet[j]);
            polygon[pos + facet[j]] = poly;
          }
        ...
        if (binCnt == 0) { // 4 points
          ...
          int[] facet = visibleFacets(pnts12, binCnt, selCnt, selectBarsOnly);
          for (int j = 0; j < facet.length; j++) {
            poly = getPolygon(pnts, facet[j]);
            polygon[pos + facet[j]] = poly;
          }
...
*/
  ...
  public boolean isFrontFacing(Polygon p) {
    ...
    for(int i = 0, n = p.npoints; i < n; i++) {
      a += p.xpoints[i] * p.ypoints[(i+1)%n] - p.xpoints[(i+1)%n] * p.ypoints[i];
    }
    return a >= 0;
  }

  public Color getBarFacetColor(int facet, int facetsDraw) {
    switch(facet % POLYS_PER_BAR) {
    case TOP_FACET:
      return facetsDraw > 0 ?  Color.BLUE : Color.darkGray;
    case FRONT_FACET:
    case BACK_FACET:
  ...
  public void paintComponent(Graphics g) {
    ...
    if (closestCorner % 4 < 2) {
      ...
      for (int i = 0; i <= displayMax; i += hIncr) {
        ...
        if (closestCorner % 4 == 0) {
          ...
          g2.drawString(s,gPnts[0].x - lw - 10, gPnts[0].y + fm.getAscent()*i/displayMax); 
        } else {
          int lw = fm.stringWidth(""+displayMax) - fm.stringWidth(""+i);
          g2.drawString(s,gPnts[1].x + lw + 10, gPnts[1].y + fm.getAscent()*i/displayMax); 
        }
      }
    }
      ...
      if (hgm != null && hgm.getModelCount() == 2) {
        if (closestCorner % 4 == 0) {
          ...
          for (int i = 0; i < bm.getBinCount(); i ++) {
            ...
            // g2.drawString(s,gPnts[0].x, gPnts[0].y + fm.getAscent()/2);
            AffineTransform xform = g2.getTransform();
            g2.rotate(rotRight, (gPnts[0].x), (gPnts[0].y + fm.getAscent()/2) );
            g2.drawString(s,gPnts[0].x, gPnts[0].y + fm.getAscent()/2);
          ...
          for (int i = 0; i < bm.getBinCount(); i ++) {
            ...
            // int yl = gPnts[0].y + 
            g2.rotate(rotLeft, (gPnts[0].x), (gPnts[0].y + fm.getAscent()/2) );
            g2.drawString(s,gPnts[0].x - lw, gPnts[0].y + fm.getAscent()/2);
            g2.setTransform(xform);
          }
          ...
          double slopeLeft = dx != 0. ? dy/dx : 0.;
          double rotRight = Math.atan2(-dy,dx); // perpendicular
          // System.err.println("slopes\t" + slopeLeft + "\t" + rotLeft + "\t\t" + slopeRight + "\t" + rotRight); 
          // right 
          bm = hgm.getBinModel(1);
          bl = bm.getBinLabeler();
          for (int i = 0; i < bm.getBinCount(); i ++) {
    ...
    /*    // Tilt is controlled so that top doesn't need to be draw
    if (closestCorner < 4) {
      // draw top
      poly = new Polygon();
      poly.addPoint(pnts[7].x,pnts[7].y);
    ...
    if (bars != null) {
      ...
      if (closestCorner % 4 == 0) {
        for (int pos = bars.length - POLYS_PER_BAR; pos >= 0; pos -= POLYS_PER_BAR) {
          for (int j = 0, k = 0; j < POLYS_PER_BAR; j++) {
            if (bars[pos + j] != null) {
              g2.setColor(getBarFacetColor(pos+j,k++));
              g2.fill(bars[pos + j]);
            }
          }
        }
        ...
        for (indices[0] = 0; indices[0] < dim[0]; indices[0]++) {
          for (indices[1] = dim[1]-1; indices[1] >= 0; indices[1]--) {
            int pos = MultiDimIntArray.getIndex(indices,dim) * POLYS_PER_BAR;
            for (int j = 0, k = 0; j < POLYS_PER_BAR; j++) {
              if (bars[pos + j] != null) {
                g2.setColor(getBarFacetColor(pos+j,k++));
                g2.fill(bars[pos + j]);
