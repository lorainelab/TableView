/*
 * @(#) $RCSfile: TypedTableModel.java,v $ $Revision: 1.7 $ $Date: 2004/08/02 20:23:47 $ $Name: TableView1_3_2 $
 *
 ...
 * the getColumnClass method such that it returns the most specific class 
 * common to all cells in the column.  The Class for a column can be explicitly 
 * set by  {@link #setColumnClass}.
 * @author       J Johnson
 * @version $Revision: 1.7 $ $Date: 2004/08/02 20:23:47 $  $Name: TableView1_3_2 $ 
 * @since        1.0
...
public class TypedTableModel extends DefaultTableModel 
  ...
  /**
   *  Constructs a TypedTableModel with <i>numRows</i> and
  ...
  /**
   *  Constructs a TypedTableModel with as many columns as there are
  ...
  /**
   *  Constructs a TypedTableModel and initializes the table
  ...
  /** 
   * Return the class hierarchy for this object.
   * the given column.
  ...
  /** 
   * Return the most specific class that is common to all Object values
   * of the TableModel column at columnIndex.
   * @param columnIndex the column being queried
  ...
  /**
   * Specify a class or interface that all Object values
   * of the TableModel column at columnIndex must extend.
   * @param columnIndex the column being queried
   * @param columnClass the java class of the values in this column.
   * @exception ClassCastException Thrown when a data element in the column 
   *            is not assignable to the given columnClass.
   */
  public void setColumnClass(int columnIndex, Class columnClass) throws ClassCastException {
    if (columnIndex >= 0 && columnIndex < getColumnCount() && columnClass != null) {
      ...
      for (int ri = 0; ri < getRowCount(); ri++) { 
        ...
        if (obj != null && !columnClass.isAssignableFrom(obj.getClass())) {
          throw new ClassCastException("At row " + ri + " " 
              + obj + " (" + obj.getClass() + ") can not be assigned to " + columnClass);
        }
      }
   ...
   * <I>aValue</I> is the new value. This will throw a ClassCastException 
   * if the value cannot be assigned to the column class explicitly set by 
   * {@link #setColumnClass}.
   *
    ...
    if (rowIndex >= 0 && rowIndex < getRowCount() && columnIndex >= 0 && columnIndex < getColumnCount()) {
      if (columnClasses != null && columnIndex < columnClasses.size() && columnClasses.get(columnIndex) != null) {
        ...
        if (aValue != null && !columnClass.isAssignableFrom(aValue.getClass())) {
          throw new ClassCastException(aValue + " (" + aValue.getClass() + ") can not be assigned to " + columnClass);
        }
   ...
   * @param column The index of the column position at which to insert the column into the table.
   * @param columnName The name of the column being added.
   * @param columnClass The java Class type of the column being added.
   */
  public void insertColumn(int column, String columnName, Class columnClass) {
    if (column < 0) {
      ...
      if (columnClasses != null) {
        columnClasses.add(column, columnClass != null ? columnClass : java.lang.Object.class);
      }
    } else {
   ...
   * @param column The index of the column to delete from the table.
   */
   ...
   * @param columns The indices of the columns to delete from the table.
   */
   ...
   * matches the column classes that were specified with {@link #setColumncClass}.
   * @param rowData The date values of a row in the table.
   * @exception ClassCastException Thrown when a data element in the row
   *            is not assignable to the given columnClass for the corresponding column.
   */
