/*
 * @(#) $RCSfile: AbstractSimilarity.java,v $ $Revision: 1.2 $ $Date: 2003/07/28 15:07:18 $ $Name: TableView1_3_2 $
 *
 ...
 *
 * @author J Johnson
 * @version $Revision: 1.2 $ $Date: 2003/07/28 15:07:18 $ $Name: TableView1_3_2
 * $
 * @since 1.0
 */
public abstract class AbstractSimilarity implements ClusterSimilarity {
    ...
    protected TreeNode cluster(List items, DoubleMatrix2D dm) {
        ArrayList clusters = new ArrayList(items); 	// work list of cluster nodes
        ArrayList nearest = null;		// list to hold nearest cluster node
        ...
        while (clusters.size() > 1) {
            ...
            if (nearest == null) {  // nearest is null, so we need to initialize
                nearest = new ArrayList();  // list to hold nearest cluster node
                ...
                for (int r = 0; r < n - 1; r++) { // loop until next to last node
                    ...
                    for (int c = r + 1; c < n; c++) {
                        ...
                        if (debug > 2) {
                            System.err.println("\tcluster comp \t" + r + "\t" + c + "\t" + tmp);
                        }
                        if (tmp < rdist) {
                            rdist = tmp;
                            rcmin = c;
                            if (debug > 1) {
                                System.err.println(r + "\tclusters found \t" + r + "\t" + rcmin + "\t" + rdist);
                            }
                        }
                    }
                    if (rcmin > 0) {
                    ...
                    if (nc == null || nc == cMin || nc == rMin) {
                        ...
                        for (int c = r + 1; c < n; c++) {
                            ...
                            if (tmp < rdist) { // this one is now the nearest
                                ...
                                rcmin = c;
                                if (debug > 1) {
                                    System.err.println("\rclusters found \t" + rmin + "\t" + rcmin + "\t" + rdist);
                                }
                            }
                        }
                        if (rcmin > 0) {
                            ...
                            nearest.set(r, nn);
                            if (debug > 0) {
                                System.err.println(" " + r + "\t recalc " + "\t" + rcmin + "\t" + (nc == cMin ? "c " + lcmin : nc == rMin ? "r " + lrmin : "null") + "\t" + rdist + "\t" + da[r]);
                            }
                            da[r] = rdist;
                        } else {
                            System.err.println("Cluster error " + r + " " + clusters.get(r));
                        ...
                        if (tmp < da[r]) {
                            ...
                            if (debug > 0) {
                                System.err.println(" " + r + "\t pn     " + "\t" + (clusters.size() - 1) + "\t" + clusters.indexOf(nc) + "\t" + tmp + "\t" + da[r]);
                            }
                            da[r] = tmp;
                            nearest.set(r, pn);
                        }
            ...
            if (debug > 0) {
                System.err.println(clusters.size() + "\tclusters merge \t" + rmin + "\t" + cmin + "\t" + dist);
            }
