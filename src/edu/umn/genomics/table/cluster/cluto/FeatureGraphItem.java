/*
 * @(#) $RCSfile: FeatureGraphItem.java,v $ $Revision: 1.3 $ $Date: 2004/02/23 17:38:49 $ $Name: TableView1_3_2 $
 *
...
/**
 * Draws bars on the Cluster graph which represent columns 
 * that are identified as defining or discrimating features 
 * by a cluto clustering solution.
 *
 * @author       J Johnson
 * @version $Revision: 1.3 $ $Date: 2004/02/23 17:38:49 $  $Name: TableView1_3_2 $ 
 * @since        1.0
...
public class FeatureGraphItem extends AbstractGraphItem {
   ...
   * @param cols an array of column indices
   */
   ...
   * Graph bars for featured columns.
   * @param cols an array of column indices
   * @param barOffset the amount (0 - 1) to offset the bar from the baseline
   * @param barHeight the size of the bar (0 - 1) relative to the height of the graph
   */
  ...
  public void draw(Component c, Graphics g, Rectangle r, Axis xAxis, Axis yAxis) {
    ...
    for (int i = 0; i < cols.length - 1; i++) {
      ...
      w = xAxis.getPosition(cols[i] + stripWidth*.5) - xAxis.getPosition(cols[i] - stripWidth*.5);
