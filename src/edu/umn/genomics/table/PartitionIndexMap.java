/*
 * @(#) $RCSfile: PartitionIndexMap.java,v $ $Revision: 1.6 $ $Date: 2004/02/27 17:17:24 $ $Name: TableView1_3_2 $
 *
...
/**
 * Provides a mapping between the array indices of 
 * a list of elements, and a partition on that list of elements. 
 * <br>For example, given partition index array :
 * <code>
 *  [ 1 2 2 0 1 0 5 3 1 5 ]
 * </code>
 * in which each of the 10 elements of the list are assigned to 
 * one of 6 parts of the partition.
 * <ul>
 ...
 *
 * @author       J Johnson
 * @version $Revision: 1.6 $ $Date: 2004/02/27 17:17:24 $  $Name: TableView1_3_2 $ 
 * @since        1.0
 */

public class PartitionIndexMap implements IndexMap, Cloneable {
 /* 
  * For effeciency sake, we sort the initial array and use a OneToManyIndexMap 
  * to reference that sorted list, then we use a OneToManyIndexMap to map 
  * from a partition into that sorted list.
  */
   ...
   * Provides a mapping between the array indices of 
   * a list of elements, and a partition on that list of elements 
   * using the given partition array. 
   *
   * @param partitionIndex The index mapping array.
   * @see #setIndex
   */
  ...
  /**
   * Return true if this is a one to one mapping of indices 
   * to partition indices.
  ...
  /**
   * Get the size of the source index, that is, 
   * the length of the array of items that were partitioned.
   ...
   * that is the number of partitions into which the 
   * array of items were assigned.
   ...
   * @return the source index mapped from the destination index, 
             or -1 if the dstIndex is not mapped to a source index.
   */
...
/**
 * The partitionIndex provides a mapping between the array indices of 
 * a list of elements, and a partition on that list of elements. 
 * The partitionIndex array is the same length as the size of the list 
 * it partitions.  The value at partitionIndex[<b>i</b>] is the part that  
 * element <b>i</b> in the list is assigned to.
 *
 * @param partitionIndex The index mapping array.
 */
  public void setIndex(int partitionIndex[]) {
    ...
    if (!asc) { // get sortIndex
      ...
      // for (int i = 0; i < si.length; i++) {
        // System.err.println("\t" + i + "\t" + partition[i] + "\t" + si[i]);
      // }
    }
    for (int i = 0; i < si.length; i++) {
      if (si[i] != i) {
