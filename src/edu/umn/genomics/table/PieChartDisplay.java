/*
 * @(#) $RCSfile: PieChartDisplay.java,v $ $Revision: 1.1 $ $Date: 2004/08/02 20:23:45 $ $Name: TableView1_3_2 $
 *
...
public class PieChartDisplay extends JComponent {
        ...
        if (arcs != null) {
            for (int j = 0; j < arcs.length; j++) {
                ...
                for (int i = 0; i < arcs[j].length; i++) {
                    if (arcs[j][i] != null) {
                        if (arcs[j][i].intersects(r.getX(), r.getY(), r.getWidth(), r.getHeight())) {
                            int[] ia = MultiDimIntArray.getIndexArray(indices, modelDim);
    ...
    public Color getColor(Arc2D arc) {
        float hue = (float) ((int) (arc.getAngleStart() + arc.getAngleExtent() / 2) % 360) / 360f;
...
//System.err.println("arcs " + j + " " + arcs[j].length + "  " + arrayString(hgm.getBinCounts()));
                for (int i = 0; i < arcs[j].length; i++) {
                    arc = arcs[j][i];
                    if (arc != null) {
            ...
            for (int j = arcs.length - 1; j >= 0; j--) {
                ...
                double radius = winSize / 4 + winSize / arcs.length * .2 * j + 5;
                if (bl != null) {
                    for (int i = 0; i < arcs[j].length; i++) {
                        arc = arcs[j][i];
                        if (arc != null) {
                            ...
                            lblArc.setArcByCenter(arc.getCenterX(), arc.getCenterY(), arc.getWidth() * .4,
                                    arc.getAngleStart() + arc.getAngleExtent() / 2, 0., Arc2D.PIE);
                            Point2D p1 = lblArc.getStartPoint();
                            lblArc.setArcByCenter(arc.getCenterX(), arc.getCenterY(), radius,
                                    arc.getAngleStart() + arc.getAngleExtent() / 2, 0., Arc2D.PIE);
                            Point2D p2 = lblArc.getStartPoint();
                            g2.setColor(Color.darkGray);
                            g2.draw(new Line2D.Double(p1, p2));
                            g2.setColor(Color.black);
                            double cos = Math.cos(lblArc.getAngleStart() * Math.PI / 180);
                            double sin = Math.sin(lblArc.getAngleStart() * Math.PI / 180);
                            int lx = (int) (p2.getX() + (sw * (cos - 1) * .5) + fm.getMaxAdvance() * cos);
