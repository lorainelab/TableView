/*
 * @(#) $RCSfile: OneToManyIndexMap.java,v $ $Revision: 1.6 $ $Date: 2004/02/27 17:17:24 $ $Name: TableView1_3_2 $
 *
...
/**
 * Provides a mapping between two sets of indices that are 
 * in sequential order.  The source indices map to any number of 
 * destination indices.  But each destination index can map 
 * to one source index at most.  The mapping is provided by the srcToDstIndex
 * array.   The srcToDstIndex has a length that is 1 greater 
 * than the length of the source index.  
 * <br> A source index <code><b>i</b></code> maps to destination indices 
 * <code>srcToDst[<b>i</b>]</code> to <code>srcToDst[<b>i</b>+1]</code>.  
 * The number of destination indices equals 
 * <code>srcToDst[<b>i</b>+1] - srcToDst[<b>i</b>]</code>.
 * <br>
 * <br>
 * For example given srcToDstIndex array: <code> [0 2 5 6 6 8] </code>
 * the following shows the mapping from the 5 elements in the src to 
 * the 8 elements in the dst index.
 * <pre>
 * <br>src      [0 1 2 3 4]
...
public class OneToManyIndexMap implements IndexMap, Cloneable {
   ...
   * Provide a mapping between two sets of indices.
   *
   * @param srcToDstIndex The index mapping array.
   * @see #setIndex
   */
   ...
   * in sequential order.  The source indices map to any number of
   * destination indices.  But each each destination index can map
   * to one source index at most.  The srcToDstIndex length is 1 greater than
   * then length of the source index.  A source index i maps to
   * destination indices srcToDstIndex[i] to srcToDstIndex[i+1].  The number of
   * destination indices equals srcToDstIndex[i+1] - srcToDstIndex[i].
   *
   * @param srcToDstIndex The index mapping array.
   */
