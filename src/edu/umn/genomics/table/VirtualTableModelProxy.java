/*
 * @(#) $RCSfile: VirtualTableModelProxy.java,v $ $Revision: 1.13 $ $Date: 2004/08/18 13:32:31 $ $Name: TableView1_3 $
 *
...
/**
 * A VirtualTableModelProxy object provides a proxy to a TableModel, allowing
 * a view of that TableModel with reordered rows and columns, and hidden or added 
 * columns.  
 * This also allows objects to have a constant reference to a TableModel instance, 
 * but have the actual TableModel supplying the data to change.
 * @author       J Johnson
 * @version $Revision: 1.13 $ $Date: 2004/08/18 13:32:31 $  $Name: TableView1_3 $ 
 * @since        1.0
 * @see  javax.swing.table.TableModel
 */
public class VirtualTableModelProxy extends AbstractTableModel 
  implements Serializable, VirtualTableModel,
             TableColumnMap {
  TableModel tm = null;
  ...
  Hashtable cMaps = new Hashtable();  // VirtualColumn -> ColumnMap
  String name = null;
        ...
        if (recalculateFormulas()) {
          ...
          if (col != TableModelEvent.ALL_COLUMNS) {
            for (int i = 0; i < columns.size();i++) {
              VirtualCell cell = (VirtualCell)columns.getElementAt(i);
              if (cell instanceof ColRef && ((ColRef)cell).getColumnIndex() == e.getColumn()) {
                fireTableChanged( new TableModelEvent(getThisSource(), e.getFirstRow(),
                                                      e.getLastRow(), i, e.getType()));
              }
            }
          } else {
            fireTableChanged( new TableModelEvent(getThisSource(), e.getFirstRow(),
   ...
   * @param name a name for this VirtualTableModelProxy.
   * @see #getName()
   * @see #toString()
   */
  ...
  /**
   * Return a String representation for this instance, which will 
   * be the name if it has been given.
   * @see #setName(String name)
   * @see #getName()
   */
  ...
  /**
   * Create a new instance of VirtualTableModelProxy, call setTableModel 
   * set the TableModel which this instance will reference.
   * @see #setTableModel(TableModel tableModel)
   */
  public VirtualTableModelProxy() {
    columns.addListDataListener(
      new ListDataListener() {
        public void intervalAdded(ListDataEvent e) {
          if (notify) {
            recalculateFormulas();
            fireTableChanged(new TableModelEventExt(getThisSource(), e.getIndex0(),
                                                    e.getIndex1(), TableModelEvent.INSERT, null));
            fireTableStructureChanged();
          }
        }
        public void intervalRemoved(ListDataEvent e) {
          if (notify) {
            recalculateFormulas();
            fireTableChanged(new TableModelEventExt(getThisSource(), e.getIndex0(),
                                                    e.getIndex1(), TableModelEvent.DELETE, null));
            fireTableStructureChanged();
          }
        }
  ...
  /**
   * Create a new instance of VirtualTableModelProxy references 
   * the given TableModel.
   * @param tableModel the TableModel which this instance will reference.
   * @see #setTableModel(TableModel tableModel)
   */
  ...
  public void setTableModel(TableModel tableModel) {
    ...
    getTmColumns();
    setDefaultColumns();
    // fireTableStructureChanged(); // already notified from setDefaultColumns();
  }
   ...
   * @param name the name given this column.
   * @param formula a formula to generate values for this column.
   */
   ...
   * @param formula a formula to generate values for this column.
   * @param index at which to add the Column.
   */
   ...
   * @param name the name given this column.
   * @param columnIndex the index of the TableModel to add.
   */
  ...
  /**
   * Add a column of the source TableModel to this VirtualTableModel.
   * @param col The column to add to the TableModel.
   */
   ...
   * @param column the Column of the TableModel to add.
   * @param index at which to add the Column.
   */
  ...
  public void removeColumn(int columnIndex) {
    if (columnIndex >= 0 && columnIndex < columns.size()) {
      cMaps.remove(columns.elementAt(columnIndex));
   ...
   * @param columnIndices the index of the column to remove.
   */
  ...
  /**
   * Set an IndexMap that will allow the VirtualTableModel to present a 
   * view of the referenced TableModel with a different row ordering.
   * @param indexMap the IndexMap that reorders the rows of the TableModel
   * @see #getIndexMap()
   */
  ...
  /**
   * Get an IndexMap that will allow the VirtualTableModel to present a 
   * view of the referenced TableModel with a different row ordering.
  ...
  /**
   * Set an IndexMap that will allow the VirtualTableModel to present a 
   * view of the referenced TableModel with a different column ordering.
   * @param indexMap the IndexMap that reorders the columns of the TableModel
  ...
  public Class getColumnClass(int columnIndex) {
    if (columnIndex >= 0 && columnIndex < columns.size()) {
      if (columns.elementAt(columnIndex) instanceof ColRef) {
        return ((ColRef)columns.elementAt(columnIndex)).getColumnClass(); 
      } else if (columns.elementAt(columnIndex) instanceof VirtualColumn) {
        return ((VirtualColumn)columns.elementAt(columnIndex)).getColumnClass(); 
      }
  ...
  public void setValueAt(Object aValue, int rowIndex, int columnIndex) {
    if (isCellEditable(rowIndex, columnIndex)) {
      ...
      if (ci >= 0 && ci < columns.size()) {
        ...
        if (!(cell instanceof ColRef)) { // ColRef let underlying table notify
          if (recalculateFormulas()) {
            fireTableDataChanged();  // Don't know which cells might be affected
          } else {
    ...
    if (tm instanceof TableColumnMap && rowIdxMap == null) {
      ...
      if (ci >= 0 && ci < columns.size()) {
        ...
        if (cell instanceof ColRef) {
          cmap = ((TableColumnMap)tm).getColumnMap(((ColRef)cell).getColumnIndex());
        } else {
          cmap = (ColumnMap)cMaps.get(cell);
          if (cMaps == null) {
            cmap = new DefaultColumnMap(this, columnIndex);
            cMaps.put(cell,cmap);
          }
        }
      }
  ...
  /**
   * Notify listeners of a change in state for this TableModel.
   * @param e The TableModelEvent to send.
   */
  public void fireTableChanged(TableModelEvent e) {
    // Guaranteed to return a non-null array
    TableModelListener[] listeners = getTableModelListeners();
    // First notify DefaultTableContext.ColumnMaps 
    //  so that it can change its mapping indices
    for (int i = listeners.length-1; i>=0; i--) {
      if (listeners[i] instanceof DefaultTableContext.ColumnMaps) {
        listeners[i].tableChanged(e);
      }
    }
    // Now notify other listeners
    for (int i = listeners.length-1; i>=0; i--) {
      if (!(listeners[i] instanceof DefaultTableContext.ColumnMaps)) {
        listeners[i].tableChanged(e);
      }
...
/* For debugging Listeners
  public void addTableModelListener(TableModelListener l) {
    System.err.println("vtm add " + l.hashCode() + " " + l.getClass().getName());
    if (l instanceof edu.umn.genomics.table.VirtualTableModelProxy) {
      String s = "";
      s = null;
      try {
        s.equals(l);
    ...
    super.addTableModelListener(l);
        try {
          TableModelListener tl[] = getTableModelListeners();
          for (int i = 0; i < tl.length; i++)
            System.err.println("vtm a " + i + " " + tl[i].hashCode() + " " + tl[i].getClass().getName());
        } catch (Throwable t) {
        }
  }

  public void removeTableModelListener(TableModelListener l) {
    System.err.println("vtm rmv " + l.hashCode() + " " + l.getClass().getName());
    super.removeTableModelListener(l);
        try {
          TableModelListener tl[] = getTableModelListeners();
          for (int i = 0; i < tl.length; i++)
            System.err.println("vtm r " + i + " " + tl[i].hashCode() + " " + tl[i].getClass().getName());
        } catch (Throwable t) {
        }
  }
*/
