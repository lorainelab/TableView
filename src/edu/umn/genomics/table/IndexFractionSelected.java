/*
 * @(#) $RCSfile: IndexFractionSelected.java,v $ $Revision: 1.2 $ $Date: 2003/07/28 15:07:15 $ $Name: TableView1_3_2 $
 *
...
/**
 * Determines the fraction of values in list, or a array indexed subset 
 * of a list, that are selected.
 *
 * @author       J Johnson
 * @version $Revision: 1.2 $ $Date: 2003/07/28 15:07:15 $  $Name: TableView1_3_2 $ 
 * @since        1.0
 */
public class IndexFractionSelected implements ListSelectionListener {
   ...
   * @param rowMap an index of rows in the list that comprise this sublist.
   * @param lsm if not null, the fraction is set using the current state 
   *            of this ListSelectionModel.
   */
  public IndexFractionSelected(int[] rowMap, ListSelectionModel lsm) {
    this.rowMap = rowMap;
    this.listLength = rowMap.length;
    if (lsm != null) {
      ListSelectionEvent e = new ListSelectionEvent(lsm, lsm.getMinSelectionIndex(),
                                lsm.getMaxSelectionIndex(), lsm.getValueIsAdjusting());
      valueChanged(e);
    }
  }
  ...
  /**
   * Adds a ChangeListener to the listener list.  The
   * ChangeListeners must be notified when the fractionSelected value changes.
   *
  ...
  /** 
   * Set the fractionSelected value, and fire a change event if this 
   * is a change in the value.
   * @param fraction the fraction selected value.
   */
   ...
   * @param e the selection event that characterizes the change.
   */
  ...
  /** 
   * Returns a value between 0 and 1 which represents the fraction of the indices 
   * that are selected.
