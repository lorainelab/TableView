/*
 * @(#) $RCSfile: FileTableModel.java,v $ $Revision: 1.12 $ $Date: 2004/05/04 20:00:33 $ $Name: TableView1_3_2 $
 *
 ...
 *
 * @author J Johnson
 * @version $Revision: 1.12 $ $Date: 2004/05/04 20:00:33 $ $Name: TableView1_3_2
 * $
 * @since 1.0
 * @see javax.swing.table.TableModel
 */
public class FileTableModel extends AbstractTableModel implements TableColumnMap, Runnable {
    /*
    ...
    class Reader extends Thread {
        ...
        public void run() {
            int rowIndex = 0;
            int lastCnt = 0;
            ArrayList rv = new ArrayList(getColumnCount());
            while (true) {
...
//System.err.println(rowIndex + "\t" + rv);
    ...
    public void read(String source) throws IOException {
        ...
        ArrayList csv = collectColumnStats(0, lcnt);
...
//System.err.println( c + " hd= " + hd);
        ...
        if (dataRowOffset > 0) {
            ...
            for (int r = 0; r < dataRowOffset; r++) {
                ArrayList rv = parseRow(null, null, lines[r]);
                if (rv != null) {
                    for (int c = 0; c < csa.length; c++) {
                        if (c < rv.size()) {
                            Object obj = rv.get(c);
...
//System.err.println("setRowOffset " + row + "\t" + offset);
    ...
    private ArrayList parseRow(ArrayList rv, Class[] colClass, String line) {
        int ncol = colClass != null ? colClass.length : 0;
        if (rv == null) {
            rv = new ArrayList(ncol);
        } else {
            rv.clear();
        }
        if (colClass != null) {
            rv.ensureCapacity(colClass.length);
        }
        String vals[] = null;
        try {
            String _fs = fs;
    ...
    public static String guessFieldSeparator(String[] lines) {
        String sep = "	"; // TAB
        ArrayList sepv = getPossibleFieldSeparators(lines);
        //System.err.println(sepv);
        if (sepv.size() > 0) {
            if (sepv.size() > 1) {
                ArrayList v = new ArrayList(sepv);
    ...
    public static ArrayList getPossibleFieldSeparators(List lines) {
        String[] la = new String[lines.size()];
        return getPossibleFieldSeparators((String[]) lines.toArray(la));
    }

    public static ArrayList getPossibleFieldSeparators(String[] lines) {
        // first line
    ...
    private void getColumnStats(ArrayList rv, ArrayList csv, int rowIndex) {
        if (rv == null || csv == null) {
            return;
        }
    ...
    private ArrayList collectColumnStats(int fromLineNum, int numLines) {
    ...
    private synchronized ArrayList getRow(int rowIndex) throws IOException {
        /*
