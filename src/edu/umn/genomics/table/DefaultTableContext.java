 ...
 */
/**
 * DefaultTableContext manages TableModels and any subtables, views, or
 * selections related to those tables. The managed objects are maintained in a
 * TreeModel that may be viewed by JTree.
 *
 * @author J Johnson
...
public class DefaultTableContext implements TableContext {
    ...
    HashMap tm_vtm = new HashMap();     // tm -> vtm
    HashMap rowSelHash = new HashMap(); // tm -> rsm
    HashMap colSelHash = new HashMap(); // tm -> csm
    HashMap tblMapHash = new HashMap(); // tm -> ColumnMaps
    HashMap obj_nodeHt = new HashMap(); // obj -> node
    HashMap node_objHt = new HashMap(); // node -> obj
    static HashMap defaultViews = new HashMap(); // viewName -> viewClass
    static ArrayList defaultViewList = new ArrayList();    // an ordered list of built in viewNames
    static HashMap defaultViewIcons24 = new HashMap(); // viewClass -> 16x16icon
    static HashMap defaultViewIcons16 = new HashMap(); // viewClass -> 24x24icon
    ...
    private HashMap views = (HashMap) defaultViews.clone(); // viewName -> viewClass
    private ArrayList viewList = (ArrayList) defaultViewList.clone();  // an ordered list of viewNames
    private HashMap viewIcons24 = (HashMap) defaultViewIcons24.clone();
    private HashMap viewIcons16 = (HashMap) defaultViewIcons16.clone();
    // Locations and Sizes of frames 
    // so that recreated frames are placed in the previous location:
    HashMap frameLoc = new HashMap(); // Frame location
    HashMap frameDim = new HashMap(); // Frame size
    /**
    ...
    class ColumnMaps implements TableModelListener {
        ...
        ColumnMaps(TableModel tm) {
            this.tm = tm;
            maps = new ArrayList(tm.getColumnCount());
        ...
        public synchronized ColumnMap getColumnMap(int columnIndex) {
            ...
            if (columnIndex < tm.getColumnCount()) {
                if (columnIndex < maps.size()) {
                    ...
                    maps.ensureCapacity(tm.getColumnCount());
                }
                if (cmap == null) {
    ...
    /**
     * Set the default view resources. The properties identifiers:
    ...
    /**
     * Register a TableModel viewing class. The view class must implement the
     * edu.umn.genomics.table.TableModelView interface.
     *
     * @param viewName The name this view is referred as.
     ...
     * @return the icon for this view.
     */
    private Icon getViewIcon(Class viewClass, HashMap iconHash) {
    ...
    /**
     * Return the View for the tree node, or null if the node does not represent
     * a View.
     *
     * @param treeNode The tree node for the view.
    ...
    /**
     * Retrieve the TreeNode for the given object, creating it if it doesn't
     * already exist.
     *
    ...
    /**
     * Get a VirtualTableModelProxy for the given TableModel. If the given
     * TableModel is a VirtualTableModelProxy, it will be registered and
     * returned. This routine is used to guarantee that all references to a
     * TableModel are the same.
     *
     ...
     * @param rows the selected rows of the given TableModel to include in the
     * new TableModel
    ...
    /**
     * Place the given component into a frame, make it visible, track the
     * frame's location and size, and listen for window events so that the tree
     * can be maintained.
     *
    ...
    /**
     * Add a standard toolbar with close and saveimage buttons to the frame. The
     * save button will create an image of the given view Component. The frame
     * must have BorderLayout, as the toolbar placed in the BorderLayout.NORTH
     * location.
     *
    ...
    /**
     * Get the selection model for the given TableModel from the given
     * Hashtable.
     *
     * @param tm The TableModel for which to get the selection model.
     ...
     * @return The selection model for this TableModel.
     */
    private ListSelectionModel getSelectionModel(TableModel tm, HashMap ht) {
  ...
  public TreeModel getTreeModel() {
    ...
    public boolean hasTableModel(TableModel tm) {
        return tm == null ? false : tm_vtm.containsKey(tm) || tm_vtm.containsValue(tm);
    }
