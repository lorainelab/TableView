/*
 * @(#) $RCSfile: ColumnsGraph.java,v $ $Revision: 1.5 $ $Date: 2003/08/06 17:20:52 $ $Name: TableView1_3_2 $
 *
 ...
 *
 * @author J Johnson
 * @version $Revision: 1.5 $ $Date: 2003/08/06 17:20:52 $ $Name: TableView1_3_2
 * $
 * @since 1.0
 * @see ColumnMap
 * @see TableContext
...
public class ColumnsGraph extends AbstractTableModelView
     ...
     * Math.ceil(Math.log(nrow) * 0.43429448190325176) * cw /
     * (xAxisDisp.getWidth() / (double)nrow); double nti = ti; int exp =
     * (int)Math.floor(Math.log(tn) * 0.43429448190325176); char lc = '1'; if
     * (exp < 0) { lc = Double.toString(Math.pow(tn,exp)).charAt(0); } else { lc
     * = Double.toString(tn).charAt(0); } if (lc == '1') { nti =
     * Math.pow(10,exp); } else if (lc < '5') { nti = Math.pow(10,exp) * 2; }
     * else if (lc <= '9') { nti = Math.pow(10,exp) * 5; } if (nti < 1.) nti =
     * 1.; ((LinearAxis)xAxis).setTickIncrement(nti); //System.err.println("ti "
